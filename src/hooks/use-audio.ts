...
export function useAudio() {
    ...
    function setVolume(id: SoundId, volume: SoundState["volume"]) {
        if (!hasSound(id)) {
            return;
        }

        const success = audioService.setVolume(id, volume);
    ...
    function togglePlay(id: SoundId) {
        if (!hasSound(id)) {
            return;
        }
